...
export const Architecture: React.FC = () => {
  ...
  return (
    <div ref={containerRef} className="max-w-5xl mx-auto space-y-12 pb-20 animate-fade-in">
      <Helmet>
        <title>Agent Runtime - ArkSphere</title>
        <meta name="description" content="Run and orchestrate AI agents as first-class engineering entities. Execution, scheduling, observability, and integrations in the ArkSphere Agent Runtime." />
      </Helmet>
      <div className="space-y-4 arch-header max-w-3xl">
        <h2 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white tracking-tight">{t.title}</h2>
      ...
      <div className="grid md:grid-cols-2 gap-12 md:gap-20 mt-12 px-2">
        <div className="space-y-8 role-list">
          ...
          <ul className="space-y-6">
            {t.flow.map((item, i) => {
                ...
                return (
                    <li key={i} className="role-item relative pl-6 border-l-2 border-gray-200 dark:border-gray-800">
        ...
        <div className="space-y-8 interaction-list">
          <h3 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
             <span className="w-8 h-8 rounded-lg bg-green-600 text-white flex items-center justify-center text-sm">02</span>
             {t.section2}
          </h3>
           <ul className="space-y-6">
             {t.ops.map((item, i) => {
                ...
                return (
                    <li key={i} className="interaction-item relative pl-6 border-l-2 border-gray-200 dark:border-gray-800">
                        <h4 className="text-base font-bold text-gray-800 dark:text-gray-200 mb-1">{title}</h4>
                        <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
                            {desc || item}
                        </p>
