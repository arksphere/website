...
const TEXT = {
    ...
    subtitle: "The substrate that powers the Agent Runtime.",
    runtime: "Serving layer for the runtime. Inference engines (vLLM, TensorRT-LLM) and accelerators that keep agents fast and efficient.",
    orchestration: "Distributed compute and workflow control. Ray, queueing, and orchestration that back the runtime's scheduling layer.",
    infra: "Kubernetes-native control plane: KServe, Kueue, and autoscaling to place runtime workloads on GPUs with guarantees.",
    system: "System and observability layer. APIs, dashboards, and telemetry to keep runtime and infra understandable.",
    roles: {
        label: "Role in stack",
...
const RoleIcon: React.FC<{ type: 'model' | 'app' | 'platform' | 'fullstack' }> = ({ type }) => {
...
const StackCard: React.FC<{ 
  ...
  <div className="bg-white dark:bg-[#1e1e1e] rounded-2xl p-8 shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col md:flex-row gap-8 transition-colors duration-300 hover:shadow-md group">
    <div className={`flex-shrink-0 w-20 h-20 rounded-2xl ${color} flex items-center justify-center text-4xl font-bold text-white shadow-lg group-hover:scale-105 transition-transform duration-300`}>
      {icon}
    </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4">
          ...
          <div className="flex items-center gap-3 px-4 py-2 bg-gray-50 dark:bg-[#151515] rounded-xl border border-gray-100 dark:border-gray-800 self-start md:self-auto">
              ...
              <div className="flex flex-col">
                  <span className="text-[10px] uppercase tracking-wider text-gray-500 dark:text-gray-500 font-semibold leading-none mb-1">{roleLabel}</span>
                  <span className="text-sm font-bold text-gray-800 dark:text-gray-200 leading-none">{role}</span>
              </div>
          </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {items.map((item) => (
          item.slug ? (
            ...
            <span key={item.label} className="px-3 py-1.5 bg-gray-100 dark:bg-[#252525] text-gray-700 dark:text-gray-300 rounded-lg text-sm font-mono border border-gray-200 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-500 transition-colors cursor-default">
              {item.label}
            </span>
          )
        ))}
...
export const Stack: React.FC = () => {
  ...
  const servingSlugs = ['vllm', 'tensorrt-llm', 'triton', 'dynamo', 'lmcache', 'llm-d'];
  const computeSlugs = ['kuberay', 'openrlhf', 'verl', 'llama-factory', 'minference'];
  const controlSlugs = ['kserve', 'seldon-core', 'kagent'];
  const observabilitySlugs = ['openllmetry', 'langfuse', 'lorax', 'lm-evaluation-harness'];

  const mapSlugs = (slugs: string[]) =>
    slugs.map((s) => {
      const p = getProjectBySlug(s);
      return p ? { label: p.title, slug: p.slug } : { label: s, slug: undefined };
    });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-20 animate-fade-in">
      <Helmet>
        ...
        <meta name="description" content="AI Native Infra that powers the ArkSphere Agent Runtime: serving, compute, control plane, and observability layers." />
      </Helmet>
      ...
      <StackCard 
        ...
        color="bg-blue-600"
        desc={t.runtime}
        items={servingItems.length ? servingItems : [{ label: 'vLLM' }, { label: 'TensorRT-LLM' }, { label: 'SGLang' }, { label: 'Ray Serve' }, { label: 'TGI' }]}
      ...
      <StackCard 
        ...
        items={computeItems.length ? computeItems : [{ label: 'Ray' }, { label: 'Temporal' }, { label: 'LiteLLM' }, { label: 'Batch queues' }, { label: 'GPU schedulers' }]}
      ...
      <StackCard 
        icon={<svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>}
        title="Control Plane"
        color="bg-indigo-600"
        desc={t.infra}
        items={controlItems.length ? controlItems : [{ label: 'KServe' }, { label: 'Kueue' }, { label: 'Volcano' }, { label: 'NVIDIA GPU Operator' }, { label: 'Autoscaler' }]}
        role={t.roles.infra}
        roleType="platform"
        roleLabel={t.roles.label}
      />
      ...
      <StackCard 
        ...
        color="bg-yellow-600"
        desc={t.system}
        items={observabilityItems.length ? observabilityItems : [{ label: 'OpenTelemetry' }, { label: 'Grafana' }, { label: 'Prometheus' }, { label: 'APIs/UI' }, { label: 'GitOps' }]}
        role={t.roles.system}
        roleType="fullstack"
        roleLabel={t.roles.label}
      />
